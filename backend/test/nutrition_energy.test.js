...
const { dailyEnergyNeed, feedingPlanForRecipe, referenceFeedingPlanForPet, recommendationScoreFromValidation } = require('../src/services/nutrition_energy');
...
test('金枪鱼南瓜脑发育按实际能量密度反推610g而不是旧算法239g，并触发食量护栏', () => {
  ...
  assert.equal(plan.minimum_density_for_reference_volume, 1.733);
...
test('7.5kg成年绝育雪纳瑞按5%参考食量计算推荐能量密度而不是沿用候选食谱密度', () => {
  const plan = referenceFeedingPlanForPet({
    age_months: 28,
  ...
  assert.equal(plan.daily_kcal, 308);
  assert.equal(plan.daily_grams, 375);
  assert.equal(plan.per_meal_grams, 188);
  assert.equal(plan.daily_food_weight_pct_body_weight, 5);
  assert.equal(plan.kcal_per_gram, 0.821);
});
