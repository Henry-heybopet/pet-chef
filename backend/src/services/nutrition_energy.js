...
function feedingPlanForRecipe(pet, recipe, ingredientMap = ingredientsDb) {
  ...
  const referenceGrams = weight ? weight * 1000 * referencePct / 100 : null;
  ...
  const requiredDensity = referenceGrams && need.daily_kcal ? need.daily_kcal / referenceGrams : null;
  ...
  return {
    ...
    minimum_density_for_reference_volume: requiredDensity === null ? null : Number(requiredDensity.toFixed(2)),
