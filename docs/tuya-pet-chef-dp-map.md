...
- App 将同一总秒数写入 `DP7 cook_time`，页面优先使用本轮范围内的设备 `DP8 remain_time` 计算倒计时和进度；未上报或超过本轮总时长的旧值使用本地倒计时回退。
- 设备上报 `DP5=done` 或烹饪中的 `DP8=0` 后，App 只下发一次 `DP107=reset`；失败时进行一次幂等重试，并提示用户手动复位，避免完成回调重复触发 reset。
