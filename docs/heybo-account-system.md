...
- 收货地址。
- 订单。
- 第一版需要真实支付。中国区长期规划为银行卡/微信支付/支付宝，第一阶段只接微信支付；海外长期规划为信用卡/Apple Pay/Google Pay/PayPal，第一阶段只接信用卡支付（Stripe Card Payment）。
...
13. 收货地址和订单。
14. 支付接口按区域分阶段接入：中国区第一阶段微信支付，海外第一阶段 Stripe Card Payment；其他支付通道先保留规划。
