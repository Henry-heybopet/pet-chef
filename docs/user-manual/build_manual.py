...
def build():
    ...
    # Chapter 1
    chapter(doc, "第 1 章　新手机号注册、设置密码与登录")
    doc.add_heading("1.1 注册前准备", level=2)
    add_bullet(doc, "准备一个可正常使用的中国大陆 11 位手机号。")
    add_bullet(doc, "准备 6 位纯数字密码；不要使用连续数字、生日或容易猜到的组合。")
    ...
    add_step(doc, 2, "输入新手机号", "在“用户名或手机号”栏输入 11 位大陆手机号。新用户注册时必须使用手机号，不能只填用户名。")
    add_step(doc, 3, "设置 6 位密码", "在密码栏输入恰好 6 位数字。少于或多于 6 位、含字母或符号时无法继续。")
    add_step(doc, 4, "点击“新用户注册”", "系统确认该手机号尚未建立账号后，会进入“设置用户名”页面。")
    add_step(doc, 5, "设置用户名", "输入 1–18 个中文、英文或数字组合；不能含空格、标点或特殊符号，也不能全部是数字。")
    add_step(doc, 6, "完成登录", "点击“完成登录”。首页左上角由“登录”变为用户名，表示注册与登录成功。")
    add_callout(doc, "密码说明", "当前版本使用 6 位纯数字密码。请避免与银行卡、支付密码或其他重要账号共用。", "tip")
    doc.add_heading("1.3 已有账号登录", level=2)
    add_step(doc, 1, "输入账号", "可输入注册手机号，也可输入已经设置的用户名。")
    add_step(doc, 2, "输入密码", "输入注册时设置的 6 位数字密码。")
    add_step(doc, 3, "点击“登录”", "登录成功后，APP 会加载此账号名下的宠物档案、设备和使用记录。")
    add_figure(doc, "app-demo-home.png", "图 1-2　登录后的 APP 首页（“手册演示”为测试账号）")
    doc.add_heading("1.4 无法登录时", level=2)
    add_bullet(doc, "确认输入的是完整 11 位手机号或正确用户名。")
    add_bullet(doc, "确认密码为 6 位纯数字，未复制多余空格。")
    add_bullet(doc, "检查手机网络；切换移动数据/Wi‑Fi 后重试。")
    add_bullet(doc, "若连续尝试仍失败，不要反复猜测密码，应联系官方支持核对账号。")
    ...
    add_bullet(doc, "提示密码格式错误：必须正好 6 位纯数字。")
